...
- Commit `2fe0446` was pushed to `main`. Local `cargo test --locked --workspace` passed (runtime: 249 passed, 11 opt-in ignored), as did `cargo clippy --locked --workspace --all-targets -- -D warnings`, formatting and diff checks. Release workflow `35823257491` completed successfully and published v0.1.9 from `2fe04467ae14ae0c33200d0b646fd43bc776af37`. The downloaded installer SHA-256 matched its published checksum: `6b137307c016c58d9645c4122f8fd37b478a2cbfe8a39a4c3a648f81eec7697a`. The installer has not been run locally; the live test below uses the equivalent optimized daemon deployed into the existing owner.
- The old SEO task's ChatGPT page still visibly showed `Thinking failed` while its Codex turn remained in progress. After verifying that this was the only active web turn and that the configuration receipt was unchanged, the failed daemon was stopped and the optimized replacement (`SHA-256 5566c6b7c93c94e859bbebeb0abf736d2ecf8e4a14a07cc19fe7576c93c56078`) was installed into the existing owner. The scheduler restarted it with the same browser profile and unchanged configuration. This interrupted the old failed turn; it did not replay its SEO request.
- The original task `01a0c5a9-3349-79c3-93c3-1758e2f8f882` is now `systemError`: its last turn failed with `E_WEB_RECOVERING` during that replacement. A new ephemeral Codex CLI 0.155.1 request to the installed `webbridge/402f0b19d5610fcbee25b681` model returned the exact expected `CXWEB_LIVE_OK` response. Post-request health showed zero active web turns and healthy browser, authentication, web models, CLI request and configuration. This verifies a new ordinary CLI request after deployment, not recovery of the original task or a real-site `Thinking failed` retry.
- An opt-in integration test now exercises the bundled browser observer in an actual installed Chrome using an isolated, unauthenticated fixture profile. A rendered `Thinking failed` status is attributed to the submitted user turn and classified as `E_CHATGPT_THINKING_FAILED`; hiding it removes the signal. The failed page closes, and a fresh page produces a completed answer. The test passed, as did the workspace test suite, Clippy with warnings denied, formatting and diff checks. This verifies the real browser/DOM seam and page turnover; the deterministic coordinator tests verify the retry contract. It does not inject a failure into the user's ChatGPT account or prove that the third-party service will recover on every attempt.

### Keep confirmed browser cleanup within the retry path

- Found a concrete timing conflict in v0.1.9: the retry coordinator abandoned page release after two seconds while the browser adapter itself allows five seconds to confirm target destruction, plus bounded CDP operations. A successful cleanup between those deadlines would therefore produce `E_WEB_CLEANUP_UNCONFIRMED` instead of the intended fresh-page retry.
- The coordinator now allows the adapter up to 60 seconds to complete bounded cleanup, and no longer calls its separate Stop operation before releasing a page whose generation has explicitly failed. Retry still requires positive cleanup confirmation; a timeout, failed close or uncertain submission never permits a second Send. A regression with confirmed cleanup after 2.1 seconds proves the former deadline would have blocked recovery.
- The actual failed SEO prompt was not retained and its upstream cause remains unknown. The current encoded-byte ceiling is not a measured ChatGPT token capacity. The competitor also checks per-message token and composer limits, but those measurements do not establish that this particular `Thinking failed` was a context overrun; changing the limits without an account-specific qualification could reintroduce the earlier `E_CONTEXT_BUDGET` failure.
- Local `cargo test --locked --workspace` passed, including 250 runtime tests (11 opt-in ignored). The delayed-release regression passed separately, as did `cargo clippy --locked --workspace --all-targets -- -D warnings`, formatting and diff checks. Commit `f59a076` was pushed to `main`. Windows release run `35825609836` passed and published v0.1.10 from `f59a0767a0da6b3367d1a72a590a6e5afbcb8374`; the downloaded installer SHA-256 matched its published checksum: `5c16a2dcb60e634b20ba499d6c1e18c1d37defa0bc9234513cf0763e64756e39`.
- At zero active web turns, the optimized daemon was installed into the existing private owner with the same browser profile and unchanged Codex configuration hash. Installed SHA-256 is `38e2f00802196f0b5bc6bf07ca6ec2910601521d2380bc78834b0894d42abe4e`. Passive browser, account and model health recovered after restart.
- A new ephemeral Codex CLI request to the installed web model called the native shell tool to read an ignored test file and returned its exact previously undisclosed contents (`CXWEB_TOOL_ROUND_ccc655d0fe29463a91b80dcd18ad1afd`). The subsequent health snapshot showed a healthy CLI request, browser, sign-in and web models with zero active web turns. This tests a real native tool round after deployment; it does not force an actual ChatGPT `Thinking failed` event.
- With zero active web turns and a passing read-only uninstall preflight, the downloaded v0.1.10 installer completed in ordinary passive upgrade mode (`/P`) with exit code 0. The Codex configuration hash and private daemon PID remained unchanged. The new desktop binary in `%LOCALAPPDATA%\Programs\cxweb` reports product version 0.1.10; the Desktop and Start Menu shortcuts target that installation. The old `%LOCALAPPDATA%\cxweb` payload remains on disk but is no longer the shortcut target. The new desktop executable was launched, and post-upgrade runtime health remained healthy with no active turn.

### Detect a terminal error after partial ChatGPT output

- The original `Thinking failed` observer required the Copy control to be absent. A partially rendered response can have Copy before ChatGPT reports a terminal failure, which would instead make the browser observer attempt to validate incomplete output or classify it as ambiguous. The reference implementation treats a visible response-error control as a retryable upstream failure; this is behavioral evidence only, and no code was copied.
- Recognize the exact visible `Thinking failed` status or ChatGPT's explicit regenerate-error control on the current turn, including when Copy or a stale Stop control is visible. Historical assistant cards, quoted user content and hidden controls are excluded. Once the terminal signal is present, discard partial answer text and public summary, suppress completion evidence and avoid UTF-16 projection errors from incomplete output. Ambiguous user or assistant attribution remains terminal and does not authorize a resend.
- JavaScript regressions cover current, historical, hidden and quoted cases. An opt-in real-Chrome fixture verified terminal classification after an assistant with Copy and an incomplete UTF-16 answer, and again with the regenerate-error control while Stop remained visible. All 161 JavaScript tests, `cargo test --locked --workspace` (runtime: 250 passed, 11 opt-in ignored), and workspace Clippy with warnings denied passed. These fixtures test detection and attribution, not an actual third-party outage or guaranteed upstream recovery.
- Commit `4986438` was pushed to `main`. Windows release run `35827481534` passed and published v0.1.11 from `4986438f4f0588a189ad435eedf27d9a30d3c9cc`; the downloaded installer SHA-256 matched its published checksum: `e225d27ef43bba79aeca3951401b84ef6e3df21642d707e9e6e94ee1424988b1`.
- At zero active web turns, the optimized daemon was installed into the same private owner. Installed SHA-256 is `fd338aa61d6ea6ef077720255771805a5e5cd9f4ad6b47adc9080b226dfd7c46`; the Codex configuration hash was unchanged. Browser, account and web models recovered after restart. A new ephemeral Codex CLI request returned exactly `CXWEB_PARTIAL_FAILURE_FIX_OK`; post-request health showed a healthy CLI request and zero active web turns.
- The downloaded v0.1.11 installer completed as an ordinary passive upgrade (`/P`) with exit code 0. The installed desktop executable reports version 0.1.11, the private daemon PID and Codex configuration hash remained unchanged, and the new desktop process was launched. The original SEO task remains failed from the old runtime and was not replayed. No real ChatGPT outage occurred during this verification, so live automatic retry after an actual upstream failure remains unobserved.
- An additional ephemeral Codex CLI request used the installed web model at `model_reasoning_effort=max` (ChatGPT Pro). After a long initial reasoning phase, it issued three separate native shell calls to read isolated, ignored test files, then completed a second web response with the correct integer sum `97` and all three exact markers in file order. The output was compared with the local files after completion. This verifies live Pro reasoning and a parallel three-tool continuation without a generation deadline; it did not trigger a real ChatGPT terminal failure. Post-test health showed zero active web turns and healthy browser, sign-in, models and CLI request.

### Codex App update compatibility (2026-09-23)

- After installing a new Codex App, its running backend is `0.155.0-alpha.16` (SHA-256 `97d4d67419d0ac2f71342f9a5e850f9468aa622618de8ea823223edb9a91926a`). Read-only `model/list` through that backend and the installed home reported ten models, none under `webbridge/`, although the same user configuration retained the cxweb `openai_base_url` route and the CLI continued to see WebGPT. Runtime health marked App verification unknown. The missing `model_catalog_json` is intentional for this installed dynamic-catalog route; the previous App build worked without it. The new backend's full build was absent from the exact compatibility selection, so its catalog request could not receive the owned model.
- An isolated, signed-out process of the updated App backend accepted a generated cxweb catalog row with `webbridge/diagnostic`, selected it through `thread/start`, and completed a synthetic loopback turn. No user home, credentials or browser session was used. A separate opt-in runtime fixture ran the exact backend through HTTP and WebSocket after updating its reviewed hash; both local terminal-refusal sequences passed. These are compatibility and transport tests, not a claim about the actual GUI picker.
- Added the exact new build as an alias of the previously qualified App catalog wire format, and its observed executable hash to native preflight. This lets an existing installation receipt serve the new App without replacing the browser binding or changing native model metadata. A gateway regression checks that the new App request receives both native and owned models, while unknown App builds remain native-only. The complete Rust workspace passed (runtime: 251 passed, 11 opt-in ignored), all 161 JavaScript tests passed, and workspace Clippy, formatting and diff checks passed. Publication and installed-client verification follow.
- The optimized daemon was installed into the existing private owner while there were zero active web turns. Installed SHA-256 is `1fdfbbeadb82def05ecc91943f8e53f3391f204c4e16e1c32a5c8b4f0fea86fb`; the scheduled task restarted successfully. The updated App backend's first read retained an old native-only model cache. A temporary loopback forwarding probe changed only that probe process's provider identity, which forced a fresh model request through the installed daemon. No user cache file was removed. A subsequent unmodified App backend process listed eleven models, including the visible owned WebGPT model and its five observed reasoning choices. Runtime health then recorded a healthy App client handshake.
- The actual installed App backend `0.155.0-alpha.16` completed a live web text turn at `xhigh` in a disposable workspace. A second live turn used the ordinary native read and patch tools; the fixture verified the exact read, patch, final answer and resulting file. Both probes used the installed Codex home and subscription without configuration overrides, and verified the configuration and executable hashes remained unchanged. The GUI picker was not visually observed, so this establishes backend model discovery and execution rather than a claim about an already-open picker view.
- GitHub Actions run `35831980966` passed the Windows build, tests, installer-hook checks and publication, producing preview release `v0.1.12` from `c6cc63dc2d69f7c7cbb76a486d68c0454b28f330`. The downloaded installer matched its SHA-256 file (`9365b383858c9b89e964114e3f04ab559a7423008b1552dadb96bb14a9cd4abe`) and provenance. A passive upgrade returned exit code 0 and installed desktop version 0.1.12. The selected Codex configuration hash and private daemon PID were unchanged. The new desktop executable was launched; post-install health showed healthy runtime, browser, sign-in, web models, configuration and App request success, with zero active web turns. A fresh App backend process still listed the visible owned model and all five reasoning efforts.

### Version-independent Codex catalog compatibility (2026-09-23)

- Replace exact client build and executable-hash allowlists with the shared ModelInfo wire contract. New release and prerelease numbers no longer suppress owned models. Request version metadata must remain consistent, native response validation stays in place, and an explicitly selected executable still undergoes integrity and native RPC checks. Discovery never launches candidates automatically.
- Persist catalog qualification by format, accept existing version-1 receipts, and retain historical encryption domain labels so stored task checkpoints remain readable. Regression tests cover unseen future versions, legacy receipts and checkpoint decryption. Executable hashes are integrity evidence, not compatibility gates.
- Verification passed: `cargo test --locked --workspace --quiet` (runtime: 252 passed, 11 opt-in ignored), `cargo clippy --locked --workspace --all-targets -- -D warnings`, all 161 JavaScript tests, `cargo fmt --all -- --check`, `git diff --check`, and `cargo build --release --locked -p cxweb-daemon -p cxweb`.
- Live installed-client probes on App backend 0.155.0-alpha.16.3 and CLI 0.156.1 each completed native file read, patch, exact final-answer and on-disk checks without routing overrides. These versions are observed evidence only. Both read-only native model lists include the visible owned model and five reasoning levels. Actual GUI and interactive CLI pickers remain unobserved; the user explicitly prohibited further window control.
- A supplementary signed-out `probe-preflight.mjs` run failed its first fixture because the assessment also reported `target_permissions`. This is not a passing installation qualification. The permission guard and expected assertions were retained; no user ACL was changed to bypass it.
- Installed the final optimized daemon at zero active web turns, preserving the existing scheduled task, browser profile and Codex configuration hash. Installed SHA-256: `f3e29c20fb2a7beedfd8c9ed269fcb256085fbedf751e2454139e39fda26eb1e`. Both backend model lists were rechecked successfully afterward. No application windows were controlled or restarted. No new installer release is claimed; ordinary installer upgrades currently preserve the separately prepared runtime, so this local runtime deployment is distinct from installer delivery.

### Actual interactive CLI picker verification

- Started the installed CLI 0.156.1 in a separate tool-owned PTY, using the existing home and configuration without provider/model overrides. No desktop window was controlled. Its real `/model` view showed `ChatGPT Web · Latest (current)` alongside native models. The reasoning view showed Low/Instant, Medium, High, Extra high and More reasoning; the latter opened the native Advanced Reasoning view with Max. No selection was saved and no model prompt was sent. The test CLI exited with code 0.
- This closes the interactive CLI picker gate for the locally deployed runtime. The actual Codex App picker remains unobserved under the user's explicit no-window-control instruction.
- Installer inspection confirmed that ordinary upgrades currently replace the desktop payload but preserve the private supervised daemon. A supported runtime-update transaction is still required before claiming a newly published installer delivers this fix to existing installations. No installer upgrade was run in this turn.
- Investigated the earlier failed Windows CI run 35833322539: a known-correct arithmetic reference hit the fixture VM's 25 ms watchdog. Increased only that offline fixture watchdog to 250 ms to tolerate runner scheduling delays; source grammar, bounded inputs, correctness assertions and explicit timeout-failure handling remain intact. `node --test scripts/coding-fixture.test.mjs` passed all five tests, including actual failing/repaired/rejected child-process exit codes and timeout classification. This watchdog is unrelated to ChatGPT generation duration.

### Picker repair acceptance

- The user confirmed that `ChatGPT Web · Latest` is now visible in the actual Codex App model picker. Together with the observed interactive CLI picker and reasoning menus, this verifies both picker surfaces for the locally deployed version-independent catalog fix. The prior live App-backend and CLI read/patch probes verify execution separately. No Codex App window was controlled to obtain the user's confirmation.
- GitHub Actions run 35848169754 passed both Portable Rust contracts and Windows checks for the catalog fix. The later offline-watchdog follow-up is tracked by run 35848503566.
- This acceptance concerns the repaired local App/CLI model availability. It does not claim that v0.1.12 contains the fix or that a new installer was published. The independently prepared runtime upgrade gap remains a packaging follow-up; no new installer should be advertised as upgrading existing private runtimes until that path is implemented and verified.

### Investigating the long-running task's E_SESSION_SCOPE failure

- Read the latest failed turn in `01a0c5a9-3349-79c3-93c3-1758e2f8f882`: turn `01a0cdd9-329d-7500-ae03-1d6f9047c9b7` failed with E_SESSION_SCOPE after 1,962,737 ms. The installed runtime also reported that code. The previous picker acceptance did not establish long-task reliability.
- After restarting the idle runtime, ChatGPT explicitly required verification. The user reported the login window was already signed in and closed it; runtime health subsequently verified browser, account and models. This establishes recovery after user interaction, not the exact reason for the original scope failure.
- Found a deterministic scope-observer defect: an email already visible in the profile menu skipped settings inspection even when no workspace ID existed, while personal-context validation required that independent settings evidence. Settings are now inspected whenever the workspace ID is absent. Existing selected-account corroboration and ambiguity rejection are retained.
- A new real-Chrome fixture reproduces the menu-email/no-workspace state and verifies settings inspection, selected Account tab, one settings identity, and continued rejection of an otherwise uncorroborated default workspace. The fixture passed; browser-adapter tests passed (16 regular tests), as did adapter Clippy, formatting, diff checks and optimized build.
- Deployed the idle runtime with unchanged Codex configuration; SHA-256 `fb32b8daeba6629e01eee1471ad000469a13efbe7c24ff0b352c65980320035a`. An immediate live reasoning probe was rejected with E_WEB_RECOVERING before browser recovery finished; this is a failed probe, not a successful generation. Original long-task recovery remains unverified.
- After passive recovery completed, the live App-backend reasoning probe passed: exact arithmetic answer, public summary delivered 21,005 ms before the answer, unchanged configuration and executable. This verifies the deployed scope guard during reasoning and completion, not yet the original long-context task. A bounded no-tools diagnostic continuation was then sent to that existing task, with explicit instructions to preserve all SEO work and perform no external actions.

### Long-context continuation exposed slow Temporary Chat startup

- The original-task diagnostic turn `01a0ce33-f87a-79e1-944b-c1fd4d33e737` reached a terminal failure after 152,926 ms: E_BROWSER_TEMPORARY_LOADING. It did not return the requested marker. This is new live evidence and does not establish complete recovery from the previous scope failure.
- Temporary Chat startup used a 15-second DOM-readiness deadline. It now observes the same unsent page for up to 45 seconds, retaining the coordinator's separate 90-second preparation ceiling and leaving generation duration unlimited. A failed preparation confirms page closure. Verification challenges and ambiguous composers return explicit errors immediately; account/model qualification remains mandatory before Send.
- The readiness-policy regression passes for hydration after 17 seconds and readiness at 30 seconds, preserves terminal failure at 45 seconds, and rejects verification/ambiguity. All 17 regular browser-adapter tests and adapter Clippy passed. The preceding scope-fix source also passed the full Rust workspace (252 runtime tests, 11 opt-in ignored). An attempted local-HTTP browser fixture was rejected by the production origin guard; that guard was not relaxed and the failed fixture is not claimed as live evidence. Live long-task verification of the startup change is still pending.

### Structural evidence for the original long-context failure

- The next original-task diagnostic turn `01a0ce3d-55ce-7e11-a8dd-c9191bda0e89` failed with E_CHATGPT_THINKING_FAILED after 219,212 ms. Temporary Chat preparation progressed, but long-context completion remains unverified.
- Add best-effort ledger diagnostics containing input and rendered-prompt byte counts, compaction status, prepared execution attempt count and a fixed terminal result code. Request identifiers remain hashed; no prompt, response, account identity or tool arguments are stored. Only the newly admitted owner records its outcome, so duplicate requests cannot label an active request as failed.
- The diagnostic regression verifies hashed identifiers, structural values, fixed-code validation and preservation of the first terminal outcome. Runtime library tests passed (253 passed, 11 opt-in ignored); runtime Clippy with warnings denied passed. This instrumentation establishes evidence for the next live investigation and does not claim to fix upstream thinking failures.
- Optimized daemon build passed and was deployed only at zero active turns; installed SHA-256 `fea0c9efc7954a0a99b2674e9ffb23867ed2b916843fbb5fd69458f1f9d5b4cf`, with unchanged Codex configuration. Browser, sign-in and models recovered without user interaction. CI run `35861838938` passed for `33270a9`.
- The next no-tools original-task diagnostic, turn `01a0ce48-cbc6-7c71-b469-951612cf15ae`, has 746,579 input bytes and 900,587 rendered prompt bytes; checkpoint=false. It reached a third prepared execution attempt and remained active at the last observation. It must not be restarted while active.
- Code inspection found that both normal activation and installed recovery construct CoordinatorProvider with a catalog but without checkpoints/context-budget configuration. Native compaction exists in isolated qualification paths only. Thus the installed path sends this large ordinary request instead of invoking the existing lower operating budget and native compaction. This is a concrete production integration gap, not yet proof that size alone caused ChatGPT's thinking failure. Next work is live native compaction qualification, production integration and original-task recovery; no new generation timeout or silent history truncation is justified.

### Wire installed providers to native context compaction

- The instrumented original-task turn failed with E_CHATGPT_THINKING_FAILED after 221,853 ms and four prepared attempts. Its ordinary prompt was 900,587 bytes; repeating this request did not recover it.
- Enable the existing checkpoint codec and 256 KiB local ordinary-input budget before catalog publication in both initial activation and installed recovery. Both App and CLI use one installation-owned persistent DPAPI-protected checkpoint key/domain, allowing task continuation after runtime restart. The 1 MiB compaction ceiling remains a local guardrail, not a claim about ChatGPT capacity; generation has no deadline. The fixed compaction diagnostic reuses an installed codec instead of trying to configure it twice.
- Expanded the existing encrypted-compaction regression to use the production constructor and reopen a new provider against its saved key, verifying pending-call restoration after restart and existing cross-task rejection. The targeted regression, full Rust workspace, runtime Clippy and optimized CLI/daemon build passed.
- Live App backend 0.155.0-alpha.16.3 completed a real WebSocket seed, native checkpoint and exact recall with one checkpoint continuation and no plaintext assistant/tool history carried over; cleanup passed. A separate tool-result fixture failed its exact action policy before compaction (E_NATIVE_PROBE_ACTION), so tool-result qualification is not claimed. The first CLI test completed its seed but failed preparation of compaction with E_LOGIN_REQUIRED; cleanup passed. A fresh isolated CLI qualification is running. Production integration has not yet been deployed or proven on the original task.

### Account for mandatory context before requesting compaction

- Installed App/CLI native backend fixtures both passed real HTTP and WebSocket compaction/continuation against the runtime with a synthetic browser. Live App checkpoint recall passed earlier; live CLI qualification remained interrupted by login readiness. After the user signed in and closed the window, `runtime-web-login --finish` recovered the installed browser, account and models successfully.
- With production compaction enabled, original-task turn `01a0ce56-12e1-79a3-8632-46898b25fe77` was rejected before browser submission with E_CONTEXT_BUDGET. Structural-only diagnostics on the next request measured 75,630 developer-message bytes, 105,920 user-message bytes, 63,922 tool-definition bytes and 179 instruction bytes, before browser serialization. Mandatory retained context alone exceeded the old ordinary budget after formatting.
- Correct the budget assessment to reserve the ordinary history allowance in addition to mandatory policy/user content, current tools and unresolved calls, while preserving the absolute encoded ceiling. Historical reasoning and completed tool-discovery records no longer count as mandatory retained messages. No input is truncated or rewritten by assessment. This avoids refusing compaction merely because the client's mandatory material is larger than the history allowance, and avoids repeatedly compacting an already-short history.
- A new regression covers a large policy and registry with reducible history, successful post-summary sizing, and the unchanged absolute ceiling. Runtime tests passed (254 passed, 11 opt-in ignored), as did runtime Clippy, formatting and diff checks. Structural refusal diagnostics store only byte counts by fixed field/role, without any message content. Original-task recovery with this calculation remains to be checked after deployment.
- Deployed the mandatory-context correction at zero active turns, keeping the Codex configuration unchanged; daemon SHA-256 `920a5065d96efb7f7cb9b07e37ad08199f53134f8c2be203abbce46e102990d2`. Passive browser/account/model recovery passed. The actual App backend HTTP/WebSocket compaction fixture passed again after the calculation change.
- Original-task turn `01a0ce5d-b9cd-7d52-9dd6-804d10856da1` now received the native context-window signal rather than the generic E_CONTEXT_BUDGET. Its next diagnostic turn `01a0ce5e-0488-7890-a341-09f10630b89f` is actively running a real native compaction (checkpoint=true, 766,927 input bytes, 925,371 rendered bytes). This is the first observed production compaction of that existing task. Its completion and subsequent answer remain unverified; do not interrupt it merely for an observation timeout.
- The original task's real compaction turn `01a0ce5e-0488-7890-a341-09f10630b89f` failed terminally after 236,900 ms with E_CHATGPT_THINKING_FAILED, after four attempts at the 925,371-byte checkpoint prompt. No request is still active in that task. Production checkpoint routing works, but one-shot summarization does not recover this history.
- Both installed App and CLI backend HTTP/WebSocket fixtures passed again with the mandatory-context calculation. CI run `35864447352` passed for `58db5a6`. These transport fixtures use a synthetic browser; they do not override the failed live long-context result.
- Inspected the local reference application's context behavior without copying code. It separates per-message browser boundaries from total task context and has explicit multi-message staging for large inputs. That supports investigating bounded staged processing rather than another blind retry of the same oversized one-shot prompt. Next implementation must process every source segment, preserve task/pending-call binding and selected effort, prevent tool execution during summarization, and verify the final encrypted checkpoint plus the original-task continuation. No production completion or new release is claimed.

### Stage large native checkpoints across bounded browser prompts

- Replace one-shot browser compaction above 256 KiB with a rolling sequence. Each prompt contains at most 128 KiB of the canonical source fragment plus the authenticated prior summary, and is sized after browser escaping to stay within 256 KiB. Every source byte is processed in order with UTF-8 boundaries preserved. Native client metadata is excluded by canonical projection; pending calls and the selected route/effort remain unchanged.
- Each stage uses the existing durable coordinator, submission-attribution rules and explicit-failure handling. An uncertain submission or cancellation stops the sequence. Intermediate checkpoints are authenticated against the same task/account/workspace binding before their summaries become input to the next stage. Only the final encrypted checkpoint is returned to Codex; no tool execution is permitted during summarization and no generation deadline was added.
- Regression tests cover exact Unicode source coverage, bounded prompts, preservation of pending arguments/effort, absence of private metadata, rolling prior-summary propagation, identical replay without more sends, cross-task/tampered-token rejection, restart restoration and stopping after an uncertain first send. The existing single-send replay assertion was updated to compare against the first staged send count and still requires zero additional sends on replay or invalid continuation.
- Full `cargo test --locked --workspace` passed (runtime: 256 passed, 11 opt-in ignored), runtime Clippy with warnings denied passed, and optimized CLI/daemon build, formatting and diff checks passed. Live staged compaction of the original task remains pending deployment; these tests do not establish semantic quality of a real model's rolling summary.
- Final prompt review found that generic checkpoint compilation correctly treats client instructions as historical source; staging directions therefore must not rely on that field. Added an explicit runtime-only staged-checkpoint mode and trusted header instructions that identify the original task inside fragments and require cumulative summaries. Native input JSON cannot enable this mode. The new adapter regression verifies that boundary, and the full workspace tests and workspace Clippy passed again. The optimized binaries were rebuilt before live testing.
- Deployed staged-compaction runtime with trusted staging instructions while idle; installed SHA-256 `9c86684aa81bb038ee60877e5ef141ce49a72b8d06c61e2c1c2eee40f9f75c14`, with unchanged Codex configuration. Passive browser/account/model recovery passed. CI runs `35866303868` (a7742ef) and `35866754539` (d5f5d31) passed.
- Original-task diagnostic turn `01a0ce70-45d2-77c2-bccd-35eaa42572f9` is active. Its first staged checkpoint has 160,768 input bytes and a 164,596-byte browser prompt, one execution attempt, and no terminal result at the latest ledger/read-only task observation (cursor revision 12). It is still processing; no live stage completion or original-task recovery is claimed. Do not restart this runtime/request for an observation timeout. All local build/test processes from this change completed.
- Live staged progress: the first original-task stage (164,596 rendered bytes) completed successfully with one attempt at 2026-09-23T13:31:41Z. The next stage was admitted at 13:31:47Z with 188,139 rendered bytes and is generating. This proves authentication/restoration of the first rolling checkpoint and automatic progression to stage two, not yet final task recovery. The native task remains `01a0ce70-45d2-77c2-bccd-35eaa42572f9`, last app cursor revision 12. Its selected effort is max (Pro), verified from structural turn metadata; do not substitute a faster effort.

## Observation deadline correction (2026-09-23)
- Original staged diagnostic turn `01a0ce70-45d2-77c2-bccd-35eaa42572f9` terminated with `E_BROWSER_OBSERVATION` after 685,906 ms. Its first rolling checkpoint completed; the whole compaction and continuation did not. The collapsed error cannot establish whether the outer deadline or a driver error caused this failure.
- Removed the coordinator's five-second observation deadline, which was shorter than even one browser transport operation's fifteen-second allowance. The coordinator now awaits one in-flight driver observation with cancellation; the driver retains its bounded transport operations. No generation deadline or automatic resubmission was added. Reviewed fixed observation failure codes preserve transport timeout versus DOM/operation failures without exposing underlying private text.
- Regression tests verify a twelve-second observation completes with one Send, and cancellation interrupts a pending observation with one Stop and release. `cargo test --locked --workspace` passed (runtime: 258 passed, 11 opt-in ignored); `cargo clippy --locked --workspace --all-targets -- -D warnings` passed. Authenticated live recovery remains to be verified after deployment.
- Deployed `50a6f70` to the confirmed idle private host; installed daemon SHA-256 `93866b9d8b5e8fe77cdfac66a03c41aa51348b0ce298a14787324dc6ce477078`, configuration unchanged. Optimized CLI/daemon build passed. Passive recovery verified browser, account and models at 13:44:11Z; the App completed a client handshake. No user windows were controlled.
- Original-task continuation diagnostic `01a0ce83-16ca-7b92-804d-ee26fe553307` is active with the existing Pro/max selection. It requests retained task goal/next-step recall only, with no tool calls, file changes or external actions. Its first stage is generating with 164,596 rendered bytes, one attempt and no terminal result. Last compact task cursor: `cbdefe88-5416-40f8-8638-8201b9da1da8:14`. Leave this request/runtime running. CI run `35869047444` was still in progress at the latest check. Installer private-runtime upgrade support and final original-task/native-tool acceptance remain incomplete; no new release was published.

## Native staged-checkpoint transport qualification (2026-09-23)
- Added opt-in `actual_backend_continues_after_staged_compaction`, using 160 KiB synthetic answers, a 256 KiB ordinary budget and 1 MiB compaction budget. It requires multiple browser checkpoint submissions, exactly one native compaction, an encrypted continuation, restored summary and zero requests sent to the native upstream. Client selection is an executable path with protocol preflight; no version allowlist was added.
- Ran `cargo test -p cxweb-runtime actual_backend_continues_after_staged_compaction -- --ignored --nocapture` against both installed backend executables. App `0.155.0-alpha.16.3` and CLI `0.156.1` passed HTTP and WebSocket (four scenarios total). Each scenario used three checkpoint stages and six total synthetic browser submissions. Evidence: `.local/probes/runtime-context-b8d5a1f1ca609479220f6afebd91a2eb` (App), `.local/probes/runtime-context-ddf986afdb7c02556badeb3dfd160bf3` (CLI). Executable hashes remained unchanged. Workspace Clippy, formatting and diff checks passed.
- Scope limitation: the native fixture explicitly observes one `contextWindowExceeded` turn before the next turn automatically compacts and completes. This verifies protocol recovery, not uninterrupted automatic continuation within the failing turn, and uses no real ChatGPT. Avoid claiming seamless long-task support from it. Do not fabricate provider token usage to make native auto-compaction trigger. A robust overflow path still needs qualification against this requirement.
- CI `35869047444` for the observation fix passed. The live Pro original-task diagnostic `01a0ce83-16ca-7b92-804d-ee26fe553307` remains active, first stage generating on one attempt, with no new terminal error. Installer upgrade investigation reconfirmed that current NSIS hooks preserve the private runtime but do not update its executable; no installer patch or release is claimed.

## Same-request overflow recovery (2026-09-23)
- Installed providers now summarize reducible history within the original request instead of emitting a native context-window failure first. The existing bounded, authenticated checkpoint stages produce a browser-only continuation. Native Codex receives only the requested normal response, with no fabricated usage and no model/effort change. Explicit native compaction remains supported and independently tested.
- The projection keeps all system/developer messages, the latest user instruction, top-level instructions/tools/options and the newest input item verbatim. The earlier prefix is summarized; its pending calls are authenticated and restored before the newest tool result. The complete restored call/result sequence is validated before answer submission. The final replay identity includes the entire original request, preventing identical lossy summaries from conflating distinct histories. Uncertain stage submission does not retry or submit an answer.
- Added unit/provider regressions for exact retained policy/user/result/pending calls, same-request replay without more sends, distinct original histories yielding distinct final response identities, and uncertain first-stage termination. `cargo test --locked --workspace` passed (runtime 260 passed, 13 opt-in ignored); workspace Clippy with warnings denied, formatting and diff checks passed.
- Added `actual_backend_continues_without_context_failure`. Ran it against the installed App backend and CLI, each over HTTP and WebSocket. All four native turns completed in each scenario with zero native errors, zero forwarded native model requests and unchanged backend binaries. Two overflowing requests produced seven synthetic checkpoint stages total. App evidence: `.local/probes/runtime-context-2f393870d4e8379163ca08c0c25ba96c`; CLI: `.local/probes/runtime-context-120465681de8e4fc80537e3ff5e501cf`. These are synthetic-browser transport tests, not live semantic qualification.
- Remaining limits are explicit: the existing hard input/prompt ceiling is unchanged; a newest item or retained mandatory material that cannot fit is still refused. Overflow summaries are not yet incrementally reused across different native requests, so repeated long histories can require repeated summarization. The model-generated summary remains lossy; no exact retention of summarized source is claimed.
- Live original-task test still uses the previously deployed observation fix: first stage completed at 13:50:34Z; second stage admitted at 13:50:40Z with 187,072 rendered bytes and remains active on one attempt. Do not replace/restart the host while it runs. The same-request change is not yet deployed or published as a release. Installer private-runtime upgrade delivery remains unfinished.

## Private runtime installer delivery (2026-09-23)
- Added installer command `stage-runtime-update`. It inventories current/legacy private installations, validates each integration journal and exact current-user Task Scheduler receipt/action, serializes updates with a private file lock, and replaces only the fixed owned daemon path from the adjacent installed payload. Windows `ReplaceFileW` retains a unique old-image backup; already running processes are not terminated. Source/destination identity, private access, staged bytes and final bytes are checked. Existing configuration, journal, browser profile and checkpoint key are not rewritten.
- NSIS POSTINSTALL now runs the new helper. Existing installations explicitly require Windows restart (exit 3010 and reboot flag), including an idempotent reinstall because an old mapped image may still run. First installation with no private host needs no restart. Update failures are explicit. Removed the old live-health preinstall gate: a broken or busy old host must not prevent delivery of a repair that does not stop it. Uninstall/disconnect behavior is unchanged.
- Real isolated executable replacement test passed: old mapped process survived, backup retained original bytes, target contained new bytes, idempotent replacement was a no-op, and the new target remained launchable. Initial test exposed our read handles blocking Windows replacement; those handles are now released immediately before replacement, with ancestor/source guards and serialized private updates retained. No user runtime was replaced by this test.
- Ran `cargo test -p cxweb-runtime installed_update_preserves_journal_and_scheduler_and_uses_new_payload -- --ignored --nocapture`: passed with a fresh GUI-subsystem fixture and isolated scheduled task. It verified unchanged config/journal, repeat staging, unchanged scheduler ownership and v2 execution on the next scheduled launch; its task was removed afterward. `scripts/test-windows-installer.ps1` passed 17 hook/replacement-decision scenarios, including 3010/reboot, refusal and unavailable-old-host installation. Full workspace tests passed before the added opt-in fixture; focused platform tests and workspace Clippy with warnings denied passed after the final access checks. No production update/reboot was invoked.
- User requested a separate model choice in the cxweb UI for compaction, because Pro summaries take roughly 30 minutes. This is now required scope. The inspected competitor selection path resolves the incoming model/reasoning for compaction, while its Luna-specific rolling checkpoint path reuses earlier summaries; no separate compaction-model setting was found in those inspected paths. No competitor code was copied. Next implement a persistent choice from qualified WebGPT model/effort options, apply it only to new compactions, retain the task's model, preserve checkpoint binding to the original task/route, and test UI/persistence/actual selection. Do not interrupt the active Pro compaction to apply it.
- Live original-task stage three completed at 14:09:49Z; stage four began at 14:09:55Z with 200,484 rendered bytes, one attempt. The original diagnostic turn remains `01a0ce83-16ca-7b92-804d-ee26fe553307`. Runtime is still the observation-fix build, not the new overflow/installer code. Revalidate current live state before any deployment.


## Independent compaction model selection (2026-09-23)
- Added a persistent per-installation model/effort choice in the desktop, populated only from verified reasoning families. Same as task is the default; settings can change during a turn but are read once per new compaction. Normal responses retain the task model. No user configuration, active generation or personal browser window was changed.
- Both native checkpoint requests and transparent overflow compaction select the configured route before staged execution. Request replay identity includes the choice. The encrypted checkpoint and native response model retain the original task route; unavailable choices fail explicitly before browser submission.
- Regression passed with multiple summary stages on a separate selected model/effort, original response model, successful authenticated continuation on the original route, and zero sends for an unavailable saved model. Persistence/reset/strict parsing and UI save/reset/failure restoration also passed.
- `cargo test --locked --workspace` passed (runtime 262 passed, 14 opt-in tests ignored); workspace Clippy with warnings denied passed; installed desktop behavior suite passed 35 tests. Release build and CI follow separately. This is mocked routing evidence, not yet a timed live faster-model comparison.
- Existing live Pro run continues on the previously deployed observation-fix runtime: stage four completed at 14:18:41Z, stage five at 14:27:22Z, stage six began at 14:27:30Z. No interruption or automatic retry occurred. Do not deploy over its active daemon.
- Final desktop integration check initially caught the missing Tauri command capability; added the command to the build manifest and main-local-window permission, generated its permission file with `cargo check --locked -p cxweb-desktop`, and reran the complete JS suite: 162 passed. `cargo fmt --all -- --check` and `git diff --check` passed.


## Observation transport timeout recovery (2026-09-23)
- Live original-task diagnostic failed at 14:36:44Z with E_BROWSER_TRANSPORT_TIMEOUT during stage six after five successful stages; final recall remains unverified. The old runtime is now idle. No successful long-compaction claim is made.
- Identified that CDP receive timeout and disconnected receiver both mapped to TimedOut. They now remain distinct (TimedOut versus BrokenPipe), and closed/EOF transport receives a fixed E_BROWSER_CLOSED diagnostic. A timeout during observation now only retries a read of the same leased page after a cancellable backoff. It never prepares another page, sends again, or stops solely due to the read timeout; no total generation deadline was added.
- Regressions passed for three consecutive timed-out observations followed by completion (one prepare, one Send, zero Stops), immediate terminal disconnection, and user cancellation during backoff (one Send, one Stop). Full workspace tests passed (runtime 264 passed, 14 opt-in ignored), Clippy all targets with warnings denied passed. Requested cancellation of the earlier selector-only release build so this correction can ship together.


## Faster compaction live follow-up and transport simplification (2026-09-23)
- Built the Windows installer successfully for 7ed655c and deployed its daemon only after the old original-task turn was terminal and health showed zero active work. Configuration hash was unchanged; browser/auth/models returned healthy passively, without user-window control or login. Set this installation's verified Medium option through installed_control::set_compaction using an isolated local probe; live published choices were Instant, Medium, High, Extra High, Pro. The task remains Pro.
- First Medium stage returned in about 44 seconds but failed E_CHECKPOINT_SUMMARY; this was not accepted as successful compaction. Added fixed JSON/schema/bounds diagnostics without logging response content. A second original-task diagnostic is running on that diagnostic build (daemon SHA256 9d8f593f9780593ae0b0dfb72b42ef1c022d5a44f87d29143eb03a339705f62f): first two stages completed at 14:53:27Z and 14:54:37Z; third began at 14:54:43Z. Do not interrupt it. No specific structural root cause of the first failure has been established.
- Simplified new checkpoint prompts to one JSON object in one fenced JSON block, with summary as a nested object rather than a doubly encoded string. Decoder retains legacy string support; both normalize into the same strict Summary validator and authenticated checkpoint. This removes an unnecessary escaping layer without repairing malformed JSON or accepting unknown fields, fabricated pending IDs, or nested objects in string arrays.
- Added regressions for exact quote/Windows-path/newline preservation in structured and legacy forms, rejection of extra fields/object-valued list items/invented pending IDs, and fixed private-data-free summary diagnostics. All adapter tests and the full Rust workspace passed; final Clippy/fmt checks recorded separately. The selector/timeout release attempt was cancelled before publication after the live summary error; a fresh combined release will follow qualification.
- Final workspace Clippy with warnings denied, formatting and diff checks passed for the structured checkpoint change.


## Release v0.1.14 and live full-context result (2026-09-23)
- The diagnostic-only build identified E_CHECKPOINT_SUMMARY_JSON on stage four at 14:57:38Z, confirming malformed nested summary JSON. After that turn ended and the daemon was idle, deployed 1290cf8 (daemon SHA256 8aa8fcacff8937df09c9235eee4baf9d5babe8da4b6e9f61d3da78d22d064176), preserving configuration. Browser/auth/models recovered passively. Medium preference persisted.
- The original App diagnostic turn 01a0cec7-43c8-7ca0-a3f6-62e26ca11b99 began at 14:59:20Z. All eight structured-summary stages completed with one attempt each; last completed 15:14:53Z. The App then submitted a normal (non-checkpoint) request, admitted 15:14:58Z with 151,532 rendered bytes. Native task metadata still records the same web model with effort=max (Pro); compaction preference is Medium. Full compaction took approximately 15m33s; prior Pro attempt failed after roughly 51m52s during stage six. This is one observed run, not a guaranteed latency or final semantic-recall result; its normal continuation is still running as of this entry.
- GitHub release workflow 35878130433 passed both build and publish for source 1290cf88eb41e5fee1b34e27b8cfbcc3c838cf4a. Published v0.1.14 with installer, SHA-256 and provenance. Downloaded the published installer into .local/release-v0.1.14-verified, verified its checksum and exact source provenance. Existing user windows were not controlled or restarted. Earlier incomplete release workflow attempts were cancelled and not presented as final builds.

- Final continuation completed at 15:16:35Z, one attempt, OK; the App marked native turn completed at 15:16:36Z with no error (total approximately 17m16s). Inspected only bounded local rollout assertions: final assistant answer exists, includes CXWEB_CONTEXT_RECOVERED and recalls the SEO goal; no answer content was exported. Task metadata retains effort=max. Runtime reports zero active web turns. This verifies live App compaction on Medium followed by successful Pro continuation; broader native tool-after-compaction and complete V1 qualification are not implied.


## Live native tool-result checkpoint qualification (2026-09-23)
- Revalidated clean source at ac0c12a and healthy idle installed runtime. Initial CLI tool-result checkpoint test failed before compaction with E_NATIVE_PROBE_ACTION. Added bounded structural rejection diagnostics (no raw commands, arguments, paths, model text or response IDs). A fresh diagnostic run proved the exact cause: two output items, first a valid public reasoning item, while the old fixture required output.len()==1.
- Fixed the isolated fixture to recognize only a response-bound, bounded, exact-shape public reasoning prefix followed by one action. Exact command, namespace, permission, path, patch, sequencing and replay checks remain enforced. Tests reject reasoning-only responses, extra/reordered tools, unauthorized commands, foreign reasoning IDs and extra fields. This corrects qualification; production already emits native reasoning alongside tools.
- Live CLI 0.156.1 over HTTP passed: native file read executed once, acknowledged its real result, compacted once, and recalled the exact file marker from the checkpoint. No plaintext assistant/tool history accompanied continuation, no native upstream frames were used, and cleanup passed. Sanitized persistent evidence: implementation/evidence/native-tool-checkpoint-cli-http-2026-09-23.json.
- Live App backend 0.155.0-alpha.16.3 over WebSocket passed the same real native-read/checkpoint/exact-recall sequence with one executed native tool, no plaintext old tool history in continuation, and confirmed cleanup. The selected executable was also observed in the running Codex process list; no user window was controlled. Evidence: implementation/evidence/native-tool-checkpoint-app-websocket-2026-09-23.json.
- `cargo test --locked --workspace` passed; focused fixture suite passed 11 tests; runtime all-target Clippy with warnings denied passed. Latest local diagnostic daemon SHA256 ddde75a8f873c500f190bef1bc8fdcb3b10a91d2ad403f48dd59a46a6674ae5b. Each deployment first confirmed idle, and configuration hashes were unchanged. Released v0.1.14 is unchanged; this additional source fix affects the qualification harness. Broader V1 completion is not inferred from these focused live tests.

## Native coding after compaction and delayed account rows (2026-09-23)
- Extended the opt-in native tool-result checkpoint qualification: after real native file read and exact checkpoint recall, Codex must apply one exact patch using the remembered value, execute the exact comparison test, and return success only after its actual successful result. The verifier checks native event attribution/order, exit status/output and both on-disk files. It never executes model tools itself or modifies user projects.
- The first continuation still must contain no plaintext prior assistant/tool history. Later requests legitimately contain the newly recalled answer and newly executed patch/test; reports distinguish those from the first continuation. Tests reject missing, duplicated, reordered or foreign-turn tools, wrong commands/results and wrong file bytes.
- Initial live CLI attempt failed before compaction submission with E_SESSION_SCOPE: structural evidence showed the account submenu was mounted but still had no controls/selected account after the previous two-second wait. Increased its bounded UI-readiness wait to ten seconds, retaining every account/workspace corroboration requirement. This is not a generation deadline. A real offscreen Chrome regression with deliberately delayed three-second row hydration passed; insufficient workspace evidence is still refused.
- Live CLI 0.156.1 HTTP qualification passed all stages with three native tools executed, one compaction, four continuations, no transport failures and confirmed cleanup. Evidence: implementation/evidence/native-post-compaction-tools-cli-http-2026-09-23.json. Live App backend 0.155.0-alpha.16.3 WebSocket qualification also passed the full sequence with three native tools, one compaction, four continuations, seven WebSocket requests, no transport failures and confirmed cleanup. Evidence: implementation/evidence/native-post-compaction-tools-app-websocket-2026-09-23.json.
- Validation passed: cargo test --locked --workspace; cargo clippy --locked --workspace --all-targets -- -D warnings; all 162 JavaScript tests; cargo fmt --all -- --check; git diff --check; the opt-in delayed-account real-Chrome regression. Deployed only while idle, preserved the user's Codex configuration, and did not control user windows. Diagnostic daemon SHA256 485112ab68b8449aea2d295b4b87b65bea3d96c4d5a38887cde5c643f4162998.
- Published Windows preview v0.1.15 from 3f0dff829fe4b9ca1fed4f3395de17d2bd541d3a. Release workflow 35884953545 completed successfully, including installer hook verification and publication. Downloaded all published assets, checked installer SHA256 8e2b99335abb0f1133d0495c7da2878402cb99262bc5cff2afbcfb09d2ecc4ea and exact source/run provenance. Existing unsigned-preview limitation remains; no user windows were controlled or installer launched. Both source and evidence CI passed. Rechecked original task 01a0c5a9-3349-79c3-93c3-1758e2f8f882: latest diagnostic turn remains completed with no error.

## Reinstallation must return to setup (2026-09-23)
- User-installed v0.1.15 after uninstall reproduced RemovalPendingRestart in the real installed runtime, with no active turns and no browser/auth evidence. The desktop inventory intentionally kept the old native-only compatibility listener, but startup incorrectly treated it as an active installation and hid onboarding. Check status only refreshed that terminal removal state.
- Inventory now reports connection_removed only after validating the removal journal against current configuration. Desktop startup excludes those verified removed connections and returns to its normal login controller when none remain. Active/reintroduced routes and journal errors still prevent competing setup. Legacy records without this new field are treated as active.
- Completed removal also recognizes a replacement route without claiming its model selection or tools. A changed route is never permission to undo a foreign connection; an exact old owned MCP entry still prevents skipping the old record. This avoids an old removal journal blocking attachment after a new installation activates on the same home.
- Tests cover real fixture-host disconnect, stopped compatibility listener, reintroduced old routing, replacement installation on the same home with byte-preserved configuration, foreign routes, old MCP entries, startup after removal, mixed active/removed inventories and ownership errors. All 165 JavaScript tests, cargo test --locked --workspace and workspace all-target Clippy with warnings denied passed. No user configuration/profile was deleted or restored as a workaround, and no user window was controlled.
- Ran the new production inventory code read-only against the user's actual preserved installation: records=1, active=0, removed=1, diagnostics=0. The login service independently reported disconnected, routing_installed=false and background_session=false. Closed only the idle login helper created by this check; the user's desktop and installed compatibility runtime were not controlled. The new desktop GUI itself was not launched under the user's window-control restriction.
- Published v0.1.16 from 096790467d662550cd515389baa0e347415a5c2d. Source CI and release workflow 35888193829 passed, including packaging and installer hook tests. Downloaded the published installer and verified SHA256 6124a879f8e1c30b3701de38d74fb6b344702f1b06bab4c816af9da1819afbc8 plus exact source/tag/run provenance. The user's installation of this artifact and interactive sign-in remain user-side verification; no successful GUI sign-in is claimed by these checks.

## Detect and prevent a mixed desktop payload (2026-09-23)
- After the user installed v0.1.16 and restarted, the actual running desktop path still contained PE version 0.1.15 with SHA256 d4ba739011ac8e47af33e2f3b454a2e178d89fa9060ee00c207f30ca4127938e. The adjacent CLI and daemon matched the extracted v0.1.16 release exactly; uninstall.exe reported 0.1.16. The published installer itself contains desktop version 0.1.16, SHA256 8d7abe668cae763a228ce0953bc926dabcda4527a063c1cb28f3adaf40f4861d. This proves a mixed installed payload; the historical reason the original NSIS File operation left the desktop unchanged is not established.
- Repaired only the installed desktop file using the verified published payload and atomic ReplaceFileW, preserving the old file as cxweb-desktop.before-v0.1.16.exe. Verified the replacement hash/version. Did not close or control the user's running window; requested that the user reopen it to load the new image.
- Installer now stages the desktop, reads its PE version, atomically replaces an existing image (or renames on first install), and verifies the resulting PE version before installing resources or recording success. Failed extraction/replacement/verification aborts explicitly. Added actual NSIS payload tests for fresh install, old file, a mapped harmless PE process, a locked NSIS payload and a directory blocking the target. All 22 installer scenarios passed; successful cases verify exact file hashes, and blocked cases cannot write the success receipt. No production app is executed by these fixtures.
- User confirmation established that closing/reopening the window had only hidden/restored the same 18:41 desktop process. The tray lacked an exit action. With explicit one-time permission, stopped only that exact desktop image and launched the verified v0.1.16 executable; the new process is distinct. The separate login controller now reports disconnected with no installed routing or background session. Native Codex and supervised runtime were not restarted.
- Published and checksum-verified v0.1.17 (installer SHA256 af46c5f962fd9d4462e7981cb3f6921c0f3598371ab891c24e5878411f21c325), source f6471ec, workflow 35891339894 successful. Add an explicit Quit cxweb window tray action so users can really restart the panel; it exits only the desktop process, leaving the separately supervised runtime and Codex requests alive. cargo check --locked -p cxweb-desktop passed.

## Confirmed reinstallation recovery and v0.1.18 (2026-09-23)
- User confirmed that the corrected desktop opened login, sign-in completed and the login Chrome window was closed. Production background/refresh/discovery recovered the authenticated session and 15 observed candidates. Live Extra High text and tool-envelope qualification passed; connect-codex restored the route through the production activation API without manual configuration repair.
- Installed-family qualification then passed text and tools for Instant, Medium, High and Pro; Extra High was already qualified. Both actual native CLI and App backend model/list APIs returned ChatGPT Web · Latest with Instant, Medium, High, Extra High and Pro, alongside eight native models. Restored Medium compaction through the production installed-control API. No user windows were controlled during these checks.
- Real ephemeral read-only CLI smoke returned its exact fixed response and exit code 0. Its first process inherited this desktop task's originator override, so it was attributed to App transport. Repeated the fixed smoke with only that child environment override removed: standalone CLI returned its exact response and exit code 0. Final production health reports overall=ready, all eight components healthy and zero active web turns. App picker evidence is from its real backend; no new GUI interaction is claimed. Sanitized evidence: implementation/evidence/reinstallation-recovery-2026-09-23.json.
- Release workflow 35892799211 passed build, all installer scenarios and publication for source f1bfb4c335dff649ca70864c479e1ee09cbb38ce. Published v0.1.18; downloaded installer checksum is 304328def7683bf7718099fcf28a476e28a769d571d7b5b439b782ebba3b3f23. Verified exact source/run provenance and extracted staged desktop PE FileVersion/ProductVersion 0.1.18. This release includes atomic verified desktop installation and the explicit Quit cxweb window tray action. The user's currently running repaired desktop remains v0.1.16; the new installer was verified but not silently installed or relaunched.

## Long-running task scope failure investigation (2026-09-24)
- Confirmed task 01a0d008-3ab7-7542-8d84-0fe6362657a4 ended with E_SESSION_SCOPE. Read-only production ledger inspection shows the failed checkpoint began at 23:38:27Z on September 23 and failed at 23:38:45Z with attempts=0. Five immediately preceding checkpoint stages completed successfully once each. The failed stage did not send a message. Original account-menu structural evidence was not persisted, so an actual identity change versus an incomplete UI read remains unproven.
- Found and fixed a concrete confirmation defect: a first incomplete account observation taking at least ten seconds skipped the intended second read, even though the adapter itself allows ten seconds for submenu hydration. Confirmation now permits exactly one additional bounded read regardless of first-read duration. Positively observed identities are still compared outside this retry, and rate limits/transport failures are not retried. No Send or generation is retried by this change. A regression with a 10.05-second incomplete first read now passes; existing mismatch, restriction and attempt-bound checks remain intact.
- Added private structural scope-failure diagnostics for generation and startup recovery. Incomplete observation and positive identity mismatch are distinguishable; mismatch exports only comparison booleans, never account/workspace values or hashes. The original failure cannot be retroactively diagnosed by this addition.
- After independently confirming zero active web turns and the expected terminal scope failure, atomically installed the initial diagnostic-only daemon (SHA256 2bb033cc0592dbfbd9ae02f692b910ed8f92211566e0f0f9cc6e542e06f32022), stopped only its exact installed owner and restarted through its existing scheduled task. User configuration hash remained unchanged; native Codex and MCP children were not stopped. Cold browser recovery now reports E_BROWSER_VERIFICATION_REQUIRED before scope inspection. Asked the user to complete the provider verification through cxweb; no challenge was automated or bypassed. This does not prove that verification caused the original failure.
- cargo test --locked --workspace passed (runtime 269 passed, 14 opt-in ignored); workspace all-target Clippy with warnings denied, formatting and diff checks passed. The updated release daemon built successfully. Final retry correction has not yet replaced the diagnostic-only running daemon or been qualified against the authenticated live task. No fixed-release or full robustness claim is made; provider verification and live compaction continuation remain outstanding.

## Verified scope recovery and release v0.1.19 (2026-09-24)
- User completed provider verification and closed login. Production finish-login recovered the same authenticated binding. After confirming idle, atomically installed the final daemon (SHA256 16f1f951d5c5b1109bdfd13c87f2e5e642275b84ecd529bd388a0d938c8e9f94) and restarted only its exact installed owner through the existing scheduler registration. Configuration hash remained unchanged. Background recovery succeeded without another login or user-window control.
- Live App backend WebSocket qualification passed native file read, exact tool-result recall after compaction, subsequent native patch and actual comparison test: three tools, one compaction, four continuations, zero transport failures and confirmed cleanup. Live CLI HTTP qualification passed the same sequence. Both verify the first checkpoint continuation carries no plaintext assistant/tool history. Reports: implementation/evidence/scope-fix-native-app-compaction-2026-09-24.json and scope-fix-native-cli-compaction-2026-09-24.json.
- Sent one explicitly diagnostic, tool-free continuation to the original failed task, preserving its selected web model and effort=max. Native turn 01a0d1ea-6f07-7ec3-b30d-dbbac4d6538a completed with no error in 804195 ms. All seven Medium checkpoint stages and the normal Pro continuation completed with attempts=1. The final answer contains CXWEB_SCOPE_RECOVERED and correctly recalls the original SEO goal. Original-task evidence is structural only: implementation/evidence/original-task-scope-recovery-2026-09-24.json. No SEO actions were requested by the diagnostic continuation. This is a complete long-context recovery run, not a fresh three-hour soak or proof of the historical DOM failure's exact shape.
- Opt-in real offscreen Chrome test menu_email_does_not_skip_personal_context_settings_evidence passed; it verifies delayed submenu hydration while still refusing insufficient personal-workspace corroboration. Source CI 35959850365 and follow-up CI 35960710561 passed on Windows and Linux.
- Release workflow 35961054883 passed build, installer tests and publication for 5c77fe5b254f3995a8cba30d5985a69d405db190. Published v0.1.19, downloaded the installer, verified SHA256 c3e8a4be561f97bb7960217f88ed88306c15aed6b42171e0c41221b732a6a8ee and exact source/run provenance; extracted desktop PE version is 0.1.19. Final runtime health at 05:53:09Z reports ready, all eight components healthy and zero active web turns. The running runtime is already corrected; the installer was not silently run. Provider-requested human verification remains a user action, never bypassed by this fix.

## Runtime-owned pending execution state (2026-09-24)
- Investigated E_CHECKPOINT_PENDING_TOOLS on the user's continued App task: the failed checkpoint had one browser submission, after several completed stages and a completed normal response. Exact model output was not retained, so a particular wrong ID is not claimed as proven. The old model-summary contract nevertheless required probabilistic reproduction of native execution identifiers, despite separately preserving every pending call in the authenticated checkpoint.
- New model summaries contain only six prose fields. The runtime derives unresolved_tool_ids from validated native history before sealing. Existing authenticated checkpoint format and strict restore validation remain unchanged. Explicit legacy model-supplied IDs still require exact agreement; malformed histories, fabricated IDs, duplicate calls/results, wrong result kinds and unknown fields remain rejected.
- Added prose-summary regressions and moved provider integration fixtures to the new contract. These exercise staged summaries, checkpoint replay, unresolved custom-call preservation, later denied results, restart recovery and subsequent compaction. Extended the opt-in synthetic live probe to carry an unresolved custom call across the checkpoint and accept its explicitly synthetic denied result only after restoration; this is not native execution evidence.
- Validation passed: cargo test --locked --workspace; cargo test --locked -p cxweb-codex-adapter; cargo test --locked -p cxweb-runtime compaction (12 passed, one opt-in ignored); cargo clippy --locked --workspace --all-targets -- -D warnings. Release CLI/daemon build passed. Deployed daemon SHA256 33fd7ad206a8b529a039d05c450ab619bfb3310ff71474ef308d9a3f633bd70d only while idle, atomically replacing the file and restarting only the installed owner. Native clients/MCP children remained running and configuration hash was unchanged. Live qualification remains pending; an initial probe during startup recovery returned E_WEB_ACTIVE before starting.
- First live synthetic attempt failed before submission on temporary-chat observation. A second attempt sealed/replayed its checkpoint and reached continuation, but the old synthetic verifier accepted only a single output item, incorrectly excluding the supported public reasoning prefix. Updated that verifier to reuse the strict response-bound reasoning validator; tests still reject missing/wrong answers, extra actions, reordered items and foreign reasoning. No success is inferred for the earlier run without retained final-answer evidence.
- First live App fixture passed native read, checkpoint recall and the exact patch but failed the final test-command step: the model returned a final message instead of a command. The strict fixture rejected it; this attempt is recorded as failed, not erased or counted as successful. Preserved sanitized attempt evidence. This is distinct from the original pending-ID rejection.
- CLI qualification reproduced the same premature final response after patch, so it is not treated as a one-off successful run. Added explicit opt-in private response capture only inside synthetic native fixtures to identify the rejected output without exporting task content. All 11 native fixture tests and runtime all-target Clippy passed. Rebuilt and deployed the diagnostic/harness daemon while idle (1e5831d5597c8bf388fb29d0a4c95d636cc8230854fb429e9c28e62be0591746); configuration unchanged.
- The opt-in diagnostic identified an additional native fixture failure precisely: the model batched the patch and its dependent test in one response, before the patch result existed. Normal transport instructions now explicitly distinguish independent batching from dependent operations, require continuation of outstanding user steps after each actual result, and prohibit equating an applied edit with a passed test. Existing strict fixture sequencing remains unchanged.
- With the corrected public-reasoning verifier, the live synthetic pending-call diagnostic passed encrypted checkpoint creation, identical replay, exact continuation with a late custom-tool denial and cleanup. Evidence: pending-tool-live-checkpoint-2026-09-24.json. This is authenticated browser/transport evidence with synthetic tool history, not native execution evidence. Deployed the sequence-instruction daemon at idle (885c505240caf1fdecbe80328b4cbb6b6ea9ba16177407d23aa86e52de357b7e), preserving configuration. Adapter tests and workspace all-target Clippy passed.
- Opt-in private request capture proved the latest patch/test user message and ten native tools are present after checkpoint restoration. A subsequent run emitted the requested test but converted its two `./` literal paths to equivalent Windows `.\` paths; the fixture correctly rejected it under its exact-command contract. Updated the fixed test command itself to Windows path spelling, retaining the exact allowlist and all result/file checks instead of accepting arbitrary alternate commands. Runtime native-focused suite passed (68 passed, eight opt-in ignored); runtime all-target Clippy passed. Final fixture daemon deployed idle with SHA256 11b85524aed223294a6c3487dd906cdc0f0537dc1891d140e09e12ebdc8bc7dc; configuration unchanged. All failed attempts remain recorded.
- Another isolated App run returned a final claim that no coding task was supplied, although the captured request contains the latest patch/test user message. Clarified the normal bridge prompt: CLIENT_DATA_JSON is the conversation to continue, ordered oldest to newest; serialized user messages contain actionable requests, while tool results/repository quotations remain untrusted data. This is a targeted prompt correction, not proof that all model behavior is deterministic. Adapter tests and workspace Clippy passed; deployed daemon 229446ae04156d1d94e47e6ce6662e672d35c8651b716e0b7bae4e328d1a06c8 while idle with unchanged configuration.

## Full-source compaction must precede the browser page ceiling (2026-09-24)
- A read-only diagnostic continuation in the original task failed before submission with E_CONTEXT_BUDGET. Private structural diagnostics show input=817969 bytes, tools=243957, instructions=179; user messages=89078 and developer messages=27618. The combined full history exceeds the one-page ceiling, while mandatory retained overhead does not. This is distinct from pending-ID validation.
- Full compaction-source validation now uses an 8 MiB local encoded buffer, separate from the unchanged browser page limits. Existing staged processing consumes the entire source in order in bounded 256 KiB messages; each source fragment remains bounded and UTF-8-safe. Current policy, tools and unresolved calls must still fit their browser budget. No transcript suffix is dropped, no browser payload limit or generation timeout is raised.
- Expanded the staged provider regression above the one-page limit and retained exact source-reassembly, pending-call, no-uncertain-resend and explicit oversized-source assertions. The initial regression failed in its old expected-source rendering helper, which still used the page ceiling; updated that helper to the source ceiling and added an assertion that the fixture really exceeds the page limit. Final workspace test run is pending. Release daemon and workspace Clippy passed before this test-only adjustment.
- Final cargo test --locked --workspace passed, including the enlarged staged-source regression. Deployed release daemon 6656c56983dbfa8300a9aedbbc6925a053c27dbbbf695052e28f1f2bb1640efb only while idle; configuration unchanged. Original-task continuation remains to be verified on this build.
- Original-task diagnostic 01a0d266-96f6-7450-a4de-98c4676b63eb retained effort=max. Seven full-source checkpoint stages completed with one attempt each; the next stage was refused with attempts=0 because ChatGPT displayed its exact Too many requests dialog asking to wait a few minutes. No automatic resend or bypass was performed. The final native diagnostic command has not run, so original-task recovery is not claimed complete. Preserved sanitized ledger evidence. The isolated post-compaction coding fixture still had premature final-answer failures; these remain reported separately from the passing pending-call transport probe.

## Completed compaction prefixes and release evidence (2026-09-24)
- Published v0.1.21 from 87e4bea966ab3e270cc6cb8b37fd6f1f841d9caf. Workflow 35972359995 passed build, installer validation and publication. Downloaded all assets; SHA256 8d3f43e4c22d0ba170a629d98529f99d279a51002b290654d9226761aefdd831 matches the published checksum and exact source/run provenance; extracted desktop PE version is 0.1.21. Release notes distinguish proven pending-call transport from incomplete full coding qualification. This version does not yet include the prefix-cache change below.
- After a six-minute cooldown, the original-task diagnostic completed another three stages then hit the provider's Too many requests restriction again before a further Send. Continued to honor the restriction; no login reset, challenge bypass or automatic generation resend was used.
- Added bounded in-memory reuse of completed, authenticated staged checkpoints. Keys bind every session scope field, original checkpoint binding/codec, exact stage bytes (including selected summarizer effort, prior summary and pending calls). Only unchanged prefixes are reused across native turns. Removed future total-source byte count from per-stage metadata, since it changed every prefix when only the suffix grew; fragment boundaries and final-stage marker remain exact. All original history remains represented in order.
- Tests verify zero new Sends for identical source, fewer Sends for an appended suffix, no reuse across tasks or any changed scope/codec/input, exact source coverage and no uncertain-submission retries. Cache is limited to 128 entries/16 MiB, contains only encrypted checkpoint responses, is lost on runtime restart, and cached delivery is never fresh browser-health evidence. Workspace tests passed; Clippy initially flagged a test MutexGuard held across await, fixed by copying the observed fixture prompts before awaiting. Final workspace all-target Clippy, formatting and diff checks passed.
- After over sixteen minutes without generation requests, deployed the cache build at idle (c1f42c00474ff8eeecf7912918ec983f03f103423e45b7e6325ad1dfc337b6c2), preserving configuration and native clients. Live cache qualification and original-task completion remain outstanding.

## Preserve native delegation and reuse prose across pending-tool changes (2026-09-24)
- The cache build completed eight summary stages and a native read-only git status/log command in the original task. That command was not the requested diagnostic marker: native App delegation was represented as an unpaired codex_app/send_message_to_thread result and was not recognized as the newest follow-up. The next tool round repeated prefix stages because pending calls were included in every stage; the provider rate-limited the next send. This is not complete recovery evidence.
- Normal prompts now explicitly identify validated native App delegation entries; automatic overflow keeps the latest such entry verbatim along with current user and policy messages. Ordinary paired tool results cannot gain this classification. Delegation remains subject to native permissions and does not resolve pending calls.
- Intermediate summaries carry prose only; final checkpoints independently attach exact authenticated native pending state. Completed prefixes now survive a changed pending call. Final replay is separately bound to the original request and payload. Current tool schemas are excluded from summary source because continuation already retains them verbatim.
- cargo test --locked --workspace and cargo clippy --locked --workspace --all-targets -- -D warnings passed. Regression coverage includes changed pending call arguments, exact final restoration, delegation retention/classification, schema exclusion, unchanged full source coverage, scope isolation and uncertain-send refusal. Native backend and live browser qualification are still in progress. Cancelled release workflow 35974878167 before publication; do not treat its build as released.

- Release daemon build passed and was deployed to the idle production owner: faa1980ea7e9623f2292e9a6f5855c7ae970e7754862fea31f8a7bf75382db8b. Native App/CLI processes and configuration were preserved. Installed App and CLI backend tests both passed staged checkpoint continuation over HTTP and WebSocket with a synthetic browser.
- Live installed App WebSocket qualification passed at 08:41:22 UTC: native file read, explicit compaction, exact tool-result recall, native patch, native test command and final verification. Three native tools executed, seven WebSocket requests, one compaction and four authenticated continuations; no provider failures. Saved sanitized report live-app-checkpoint-tools-2026-09-24.json.
- The subsequent live CLI HTTP qualification reached compaction but ChatGPT refused further work with E_BROWSER_RATE_LIMITED. It did not complete and no automatic retry was made. Saved live-cli-checkpoint-rate-limit-2026-09-24.json. Original long-task recovery still needs independent live verification; no success inferred from the isolated App fixture.

- Published and downloaded v0.1.22 from code commit 47344e91dee5bc9b70fdbfe50370fd245a707934. Workflow 35976482895 succeeded. Installer SHA256 1656d0ef7fb3f769b8eff62b81e1426673e9a245256a1409469e8c9ac3ac3f1f matches its checksum; provenance pins the exact commit/run/tag; extracted desktop PE version is 0.1.22. Release notes were updated with the subsequent live evidence, including the CLI rate-limit interruption.

- Original-task verification on v0.1.22 code (turn 01a0d29d-750b-7943-87a8-3de1f54fdc79) completed six checkpoint stages between 08:52:25 and 08:53:50 UTC. The next stage returned E_BROWSER_RATE_LIMITED at 08:54:20 (attempts=1). No native diagnostic command ran. Completed prefixes remain in the current runtime memory; no resend of the interrupted stage or further automatic test was performed. Original-task end-to-end recovery is still unverified.

## Interrupted staged recovery regression (2026-09-24)
- Added a provider regression that completes two summary stages, receives E_BROWSER_RATE_LIMITED after the third submission, and then simulates service recovery. Repeating the same failed native request remains terminal without another send; an explicit new native request reuses both completed prefixes exactly once and restores the original pending call exactly in its final checkpoint.
- The focused regression passed; all 16 provider tests passed (5 opt-in native tests ignored); cargo clippy --locked -p cxweb-runtime --all-targets -- -D warnings and formatting/diff checks passed. This is test-only coverage of the already released production behavior, so the running owner was not replaced and its six completed live prefixes were preserved. Production owner creation time remains 08:39:18 UTC. Live service recovery is still not established.
